...
(function(exports, _) {
  ...
  function normalizeFrames (options) {
    ...
    _.each(options.frames, function (frame) {
      ...
      if (_.isUndefined(frame.begin)) {
        frame.begin = {};
      }

      normalizeTimestamp(frame,       'relatedTo', options.times);
      normalizeTimestamp(frame.begin, 'at', options.times);
      normalizeTimestamp(frame.begin, 'after', options.times);
      normalizeTimestamp(frame.begin, 'before', options.times);
      normalizeTimestamp(frame.begin, 'between', options.times);
      normalizeTimestamp(frame.begin, 'and', options.times);
  ...
  function renderLoop () {
    ...
    self.publish('tick', [frame, (frame.relatedTo - now) || 0]);
